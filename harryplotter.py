...
class harryplotter():
    ...
    def __init__(self, x, y, plot):
        self.x = x
        if x.name == "Date" or x.name == "date" or x.name == "DATE":
            self.x = pd.to_datetime(self.x, format=self.get_date_format(self.x[0]))
        self.y = y
        if y.name == "Date" or y.name == "date" or y.name == "DATE":
    ...
    def histogram(self):
        ...
        plt.draw()
        fig.savefig(self.path + "\/histogram X=" + self.x.name + " Y=" + self.y.name + ".png", dpi=200, bbox_inches='tight') # save the plot to file
        plt.clf() # clear the plot area
    ...
    def boxplot(self):
        ...
        fig.set_size_inches(32, 18)
        plt.draw()
        fig.savefig(self.path + "\/boxplot X=" + self.x.name + " Y=" + self.y.name + ".png", dpi=200, bbox_inches='tight')
    ...
    def autocorrelation(self):
        ...
        fig.set_size_inches(32, 18)
        plt.draw()
        fig.savefig(self.path + "\/autocorrelation X=" + self.x.name + " Y=" + self.y.name + ".png", dpi=200, bbox_inches='tight')
        plt.clf()
    ...
    def acf(self):
        ...
        fig.set_size_inches(32, 18)
        plt.draw()
        fig.savefig(self.path + "\/acf X=" + self.x.name + " Y=" + self.y.name + ".png", dpi=200, bbox_inches='tight')
        plt.clf()
    ...
    def pacf(self):
        ...
        fig.set_size_inches(32, 18)
        plt.draw()
        fig.savefig(self.path + "\/pacf X=" + self.x.name + " Y=" + self.y.name + ".png", dpi=200, bbox_inches='tight')
        plt.clf()
    ...
    def main(self):
        if self.plot == "scatterplot":
            self.scatterplot()
        elif self.plot == "lineplot":
            self.lineplot()
        elif self.plot == "histogram":
            self.histogram()
        elif self.plot == "boxplot":
            self.boxplot()
        elif self.plot == "autocorrelation":
